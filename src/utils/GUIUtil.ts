import { GUIHelp } from "./debug/GUIHelp";
...
export class GUIUtil {
    ...
    public static renderShadowSetting(open: boolean = true) {
        ...
        GUIHelp.add(setting, 'enable');
        // GUIHelp.add(setting, 'debug');
        GUIHelp.add(setting, 'needUpdate');
        GUIHelp.add(setting, 'autoUpdate'); // update shadown automatic

        GUIHelp.add(setting, 'updateFrameRate', 1, 10, 1); // frequency for shadows update
        GUIHelp.add(setting, 'type', { PCF: 'PCF', HARD: 'HARD', SOFT: 'SOFT' });
        GUIHelp.add(setting, 'pointShadowBias', 0.0001, 1, 0.0001); // Offset of point light shadow

        GUIHelp.add(setting, 'shadowSize', 10, 10240, 10); // shadow mapping Size
        setting.shadowBound && GUIHelp.add(setting, 'shadowBound', 0, 2048, 1); // shadow boundary

        GUIHelp.add(setting, 'shadowBias', 0.0001, 0.2, 0.00001); // shadow depth offset bias 
        GUIHelp.add(setting, 'shadowSoft', 0.0001, 1, 0.001); // Shadow softness
        GUIHelp.add(setting, 'pointShadowSize', 10, 10240, 10); // Point shadow mapping size

        GUIHelp.add(setting, 'csmMargin', 0, 1, 0.01); // Blend Shadow(0-1)
        GUIHelp.add(setting, 'csmScatteringExp', 0, 1, 0.01); // scattering csm Area Exponent for each level
        GUIHelp.add(setting, 'csmAreaScale', 0, 1, 0.01); // scale csm Area of all level
    ...
    public static renderUIShadow(image: UIShadow, open: boolean = true, name?: string) {
        ...
        GUIHelp.add(image.shadowOffset, 'x', -100, 100, 0.01).onChange((v: any) => changeOffset());
        GUIHelp.add(image.shadowOffset, 'y', -100, 100, 0.01).onChange((v: any) => changeOffset());
    ...
    public static renderUIPanel(panel: UIPanel, open: boolean = true, name?: string) {
        ...
        GUIHelp.add({ cullMode: GPUCullMode.none }, 'cullMode', cullMode).onChange((v: any) => {
            panel.cullMode = v;
        });
        ...
        GUIHelp.add({ billboard: panel.billboard }, 'billboard', billboard).onChange((v: any) => {
            panel.billboard = v;
        });
    ...
    static renderDebug(open: boolean = true) {
        // if (Engine3D.setting.render.debug) {

        GUIHelp.removeFolder(`RenderPerformance`);
        //debug
        GUIHelp.addFolder('RenderPerformance');
        if (Engine3D.getRenderJob(Engine3D.views[0]).postRenderer) {
            ...
            for (let i = 0; i < debugTextures.length; i++) {
                ...
                (debugTextureObj as any)[tex.name] = i;
            }
    ...
    static renderLitMaterial(mat: LitMaterial, open?: boolean, name?: string) {
        ...
        GUIHelp.add({ blendMode: mat.blendMode }, 'blendMode', blendMode).onChange((v: any) => {
            mat.blendMode = (BlendMode as any)[BlendMode[parseInt(v)]];
        });

        GUIHelp.add(mat, 'alphaCutoff', 0.0, 1.0, 0.0001).onChange((v: any) => {
            mat.alphaCutoff = v;
        });

        GUIHelp.add(mat, 'doubleSide').onChange((v: any) => {
            mat.doubleSide = v;
        });
    ...
    public static blendShape(obj: Object3D) {
        ...
        for (let key in targetRenderers) {
            ...
            GUIHelp.add(influenceData, key, 0, 1, 0.01).onChange((v: any) => {
                influenceData[key] = v;
    ...
    static renderAnimator(com: AnimatorComponent) {
        ...
        for (let i = 0; i < com.clips.length; i++) {
            ...
            (anim as any)[clip.clipName] = clip.clipName;
        }

        GUIHelp.addFolder('morph controller');

        GUIHelp.add({ anim: anim }, 'anim', anim).onChange((v: any) => {
            com.playAnim(v);
    ...
    static renderDepthOfField(post: DepthOfFieldPost, open: boolean = true) {
    ...
    static RenderColor(target: Object, name: string) {
        GUIHelp.addColor(target, name).onChange((v: any) => {
            let [r, g, b, a] = v;
            (target as any)[name] = new Color(r / 255, g / 255, b / 255, a / 255)
        })
    }

    static RenderVector4(label: string, target: Object, key: string, min: number, max: number, step: number = 0.01) {
        let components = ['x', 'y', 'z', 'w'];
        let data: { [key: string]: Vector4 } = {};
        let vec4: Vector4 = (target as any)[key];
        for (let component of components) {
            data[label + component] = (vec4 as any)[component];
            GUIHelp.add(data, label + component, min, max, step).onChange((v: any) => {
                (vec4 as any)[component] = v;
                (target as any)[key] = vec4;
            });
    ...
    static RenderVector2(label: string, target: Object, key: string, min: number, max: number, step: number = 0.01) {
        let keys = ['x', 'y'];
        let data: any = {};
        let vec2: Vector2 = (target as any)[key];
        for (let component of keys) {
            data[label + component] = (vec2 as any)[component];
            GUIHelp.add(data, label + component, min, max, step).onChange((v: any) => {
                (vec2 as any)[component] = v;
                (target as any)[key] = vec2;
            });
